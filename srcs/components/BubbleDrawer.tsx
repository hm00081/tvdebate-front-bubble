...
export default class BubbleDrawer {
    props: Props;
    bubblesPositive: Selection<SVGCircleElement, BubbleNodeType, SVGElement, BubbleNodeType> | null = null;
    rectPositive: Selection<SVGRectElement, BubbleNodeType, SVGElement, BubbleNodeType>;
    bubblesNegative: Selection<SVGCircleElement, BubbleNodeType, SVGElement, BubbleNodeType> | null = null;
    rectNegative: Selection<SVGRectElement, BubbleNodeType, SVGElement, BubbleNodeType>;
    bubblesText: Selection<SVGTextElement, BubbleNodeType, SVGElement, BubbleNodeType>;
    bubblesTextPercentage: Selection<SVGTextElement, BubbleNodeType, SVGElement, BubbleNodeType>;
    ...
    draw(simulation: Simulation<BubbleNodeType, undefined>): BubbleDrawer {
        ...
        const leafPositive = leafRoot.append('g').join('g').attr('class', styles.gPositive);
        ...
        const leafNegative = leafRoot.append('g').join('g').attr('class', styles.gNegative);
    ...
    handleTick = (): void => {
        ...
        this.bubblesText && this.bubblesText.attr('x', (d: BubbleNodeType) => d.x).attr('y', (d: BubbleNodeType) => d.y);

        this.bubblesTextPercentage && this.bubblesTextPercentage.attr('x', (d: BubbleNodeType) => d.x).attr('y', (d: BubbleNodeType) => d.y + 20);
    };
    ...
    handleMouseOver = ({ skillId, type }: { skillId: number; type: BubbleType }): void => {
    ...
    handleFocus = ({ skillId, type }: { skillId: number; type: BubbleType }): void => {
    ...
    public focus = (skillId: string, type: BubbleType): BubbleDrawer => {
        const { svg } = this.props;
        const node = svg && svg.select<SVGCircleElement>(`#bubble${skillId}${type}`).node();
    ...
    setTabbableBubbles = (type: BubbleType | null): BubbleDrawer => {
        const { svg } = this.props;
        svg && svg.selectAll<SVGCircleElement, BubbleNodeType>('circle').attr('tabindex', (d) => (!type || type === d.mastery ? 0 : -1));
        return this;
    };
    ...
    highlightType = (type?: BubbleType | null) => {
        ...
        const alpha = type ? (count.positive + count.negative) / count[type] / 2 : 1;
        bubbles.each((d) => {
